...
describe("指标单位 (八项逐一钉死)", () => {
  ...
  it.each([
    ...
    ["V", "%", "挥发分按百分比"],
    ["M", "%", "水分按百分比"],
    ["G", "点", "master schema: 粘结指数 (无量纲), 配煤师论点"],
    [
      "CSR",
      "点",
      "master schema 写的是百分数, 这里刻意用点: 合同原话「热后强度低 1 扣 10 元/吨」不带单位, 行业口语把这个差额叫个百分点即点 —— 可读性取舍, 不是漏改",
    ],
    ["Y", "mm", "master schema: 胶质层最大厚度 mm"],
    ["petro", "", "master schema: 岩相 (反射率分布度量), 没有单位, 宁可不写也不能写错"],
